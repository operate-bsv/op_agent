defmodule FBAgent do
  ...
  `FBAgent` is used to load and run "tapes" (Functional Bitcoin scripts).
  ...
  * `:strict` - Set `false` to disable strict mode and ignore missing and/or erring functions.
  ...
  and returns a prepared `t:FBAgent.Tape.t` ready for execution in an
  `:ok/:error` tuple pair.
  ...
  modified and complete `t:FBAgent.Tape.t` in an `:ok/:error` tuple pair.
